...
public class InequivalentBehaviourFCFilterOld {
	...
	private static boolean behaviourProfilesMatch(Set<FlowNode> nodes1,
		...
		for (FlowNode n1 : nodes1) {
			for (FlowNode n2 : nodes1) {
				...
				// n2.getName());
				BehavioralRelationType r1 = getRelation(profileOfProcess1, n1, n2);
				...
				FlowNode targetNode2 = nodeMappings
						...
						.filter(p -> p.getValue0().equals(n2)
								&& nodes2.contains(p.getValue1())).findAny()
						.get().getValue1();
				BehavioralRelationType r2 = getRelation(profileOfProcess2, targetNode1,
						targetNode2);
				if (r1 != r2) {
					profilesMatch = false;
				}
